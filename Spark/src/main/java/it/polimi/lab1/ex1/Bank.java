...
public class Bank {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Dataset<Row> totDepositsRenamed = totDeposits.withColumnRenamed("sum(amount)", "totDeposits");
        Dataset<Row> totWithdrawalsRenamed = totWithdrawals.withColumnRenamed("sum(amount)", "totWithdrawals");

        Dataset<Row> decreasingAccountBalances = totDepositsRenamed.join(
                        totWithdrawalsRenamed,
                        "account",
                        "outer"
                )
                .na().fill(0, new String[]{"totDeposits", "totWithdrawals"}) // fill missing values with 0
                .withColumn("balance", functions.col("totDeposits").minus(functions.col("totWithdrawals")))
                .orderBy(functions.col("balance").desc());

        decreasingAccountBalances.show();
